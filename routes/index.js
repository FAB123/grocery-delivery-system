...
router.get('/', async function (req, res, next) {
  ...
  if (req.session.loggedIn) {
   userData = { userName: req.session.user.first_name + ' ' + req.session.user.last_name, id: req.session.user._id, status: true }
  }

  let carousel = await imageHelpers.carouselImages();
  productHelpers.getAllProduct().then((products) => {
    console.log(userData)
    res.render('main/index', { company: 'Balsam Laundary', products, admin: false, carousel, user: userData });
  })
});

router.get('/detailes/:id', (req,res)=>{
  productHelpers.getProductdetailes(req.params.id).then(async(data)=>{
    let carouselImages = await imageHelpers.productcarouselImages(req.params.id)
    userData.carouselImages = carouselImages;
    res.render('main/detailes', { company: 'Balsam Laundary', data, admin: false, user: userData });
  })
})
